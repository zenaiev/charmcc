...
We study the charged current DIS  production of charm using the
xFitter program.  Recent results from the LHC have focused renewed
attention on the determination of the strange quark PDF, and the DIS
charm process provides important complementary constraints on this quantity.  We examine the current
PDF uncertainty, and use LHeC pseudodata to estimate the potential
improvement from this proposed facility.  As xFitter implements both
Fixed Flavor Number Schemes and a Variable Flavor Number Schemes, we
can compare the impact of these different theoretical choices; this
highlights some interesting aspects of multi-scale calculations.
We find the large kinematic range and high statistics from an LHeC
could substantially improve the strange PDF uncertainties. 
...
have significant impact in the modern determinations of the parton
...
The standard ``A'' variant of the fixed flavour number scheme (FFNS),
which we identify as {\bf FFNS~A}, 
uses three light flavours in both PDFs and $\alpha_s$ evolution for all scales, while heavy 
...
and cross-section predictions of the ABM(P) group~\cite{Alekhin:2012ig,Alekhin:2014sya,Alekhin:2017kpj,Alekhin:2018pai}, 
as well as in the FF3A variant of HERAPDF~\cite{Abramowicz:2015mha}, 
and implemented in xFitter through the OPENQCDRAD package~\cite{openqcdrad}.



Next, the ``B'' variant of the fixed flavour number scheme ({\bf \ffnsb}) known as the `mixed' 
scheme or~\cite{Behnke:2015qja} is used. In this scheme, the 
...
Corrections to the  $\alpha_s$ evolution involving heavy flavour loops are thus
...
will catch a  fraction of the ``large logs'' which might spoil the 
...
This scheme was used in the HERAPDF FF3B variant~\cite{Abramowicz:2015mha} and in applications of the HVQDIS program~\cite{,Behnke:2015qja}. 
...
% (or more specifically, the $\mu_r$ variation)
