...
%
  %
  %
\let\thefootnote\relax\footnotetext{Correspondence: {\tt olness@smu.edu}}
%
\end{abstract}
...
The CC charm process directly depends on the CKM matrix~\cite{Tanabashi:2018oca}.
Here, the CKM
...
%
\textcolor{red}{
The different PDF sets will result in a shift of the cross sections displayed
by approximately{\bf  ************* percent}.
********* \\
%
If we compare the cross sections at low values of $Q^2$ where the VFNS approaches  the FFNS limit, we see
that on the scale of our comparisons (c.f., Figs ****** 2 and 6-a) the effects are minimal except for the largest x bin ($x\simeq 0.25$). 
%
********** STILL NEEDS WORK HERE ***********
\\
********** ALSO COMMENT ABOUT PROFILING: NOTE HERAPDF2.0 has fixed strange, so we cannot profile this.
[See referee comment \# 9]
}
%
...
*** HELP!!! PLEASE CHECK REFEREE COMMENT HERE ****
}
Similar results
...
This contribution depends on the particular $\xbj$ value 
({\it c.f.,} Fig.~11 of Ref.~\cite{Kusina:2013slm}).
The final result will depend on the balance between the LO and SUB contributions,
so the behavior is not just a simple factor.
%
Figure~\ref{fig:thpred-ff3abfonll}-b)
displays the results as a function of $\xbj$;  
we observe that for $Q^2$ values not too large ($Q^2 \lesssim 4924$),
the VFNS is above the \ffns\ results for $\xbj \gtrsim 0.1$.
...
\textcolor{red}{\bf ************ WHAT ABOUT \ffnsb\ ****************}
...
$W^+ g \to c\bar{s}$. In contrast, in the VFNS the charm is present also
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\textcolor{red}{************ add comment and ref
\cite{AbdulKhalek:2019mps}
********** \\
...
while the additional channels yields improved constraints on the gluon. 
}
...
\bibliographystyle{spphys} \bibliography{main,extra}
